...
class Follower:
    ...
    def image_callback(self, msg):
        ...
        print "mc: {}".format(mc)
        for c in mc:
            cv2.circle(origin_back, c, 20, (255, 0, 0), -1)
            cv2.imwrite("point{}.jpg".format(self.i), origin_back)
        ...
        if len(choose_mc):
            ...
            if self.signal is True:
                if self.err < 0:
                    self.turn_cmd.angular.z = math.radians(-5)
                    self.err_total -= 5
                elif self.err > 0:
                    self.turn_cmd.angular.z = math.radians(5)
                    self.err_total += 5
                ...
                if self.err_total >= 30 or self.err_total <= -30:
                    for x in range(0, 10):
                        self.cmd_vel_pub.publish(move_cmd)
                        r.sleep()
                    self.signal = False
                else:
                    self.turn_cmd.angular.z = 0
